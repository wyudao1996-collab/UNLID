...
title_row(ws_a, 1, "UNLID 財務モデル 前提条件 v4.1",
...
# ─ セクション⑤: 解約率（Churn Rate）＆ グロス新規獲得計画
sec_hdr(ws_a, 59, "⑤ 解約率（Churn Rate）＆ グロス新規獲得社数計画（重要）")
col_hdr(ws_a, 60)

sc(ws_a, 61, 1, "月次解約率（月チャーン）")
for c, v in [(2, 0.030),(3, 0.025),(4, 0.020)]:
    sc(ws_a, 61, c, value=v, nf=NF_PCT, align="center")

sc(ws_a, 62, 1, "  年次解約率（年チャーン）＝ 1-(1-月次)^12", italic=True, fc=C_NOTE)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 62, c, formula=f"=1-(1-{col}61)^12", nf=NF_PCT, italic=True, fc=C_NOTE, align="center")

sc(ws_a, 63, 1, "期初契約社数（前年末）")
sc(ws_a, 63, 2, value=0, nf=NF_MAN, align="center")
sc(ws_a, 63, 3, formula="=B6", nf=NF_MAN, align="center")
sc(ws_a, 63, 4, formula="=C6", nf=NF_MAN, align="center")

sc(ws_a, 64, 1, "  期中解約予測社数（期初×年次解約率）", italic=True)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 64, c, formula=f"=ROUND({col}63*{col}62,0)", nf=NF_MAN, italic=True, align="center")

sc(ws_a, 65, 1, "期末契約社数（ネット目標）", bold=True, fc=C_SUB)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 65, c, formula=f"={col}6", bold=True, fc=C_SUB, nf=NF_MAN, align="center")

sc(ws_a, 66, 1, "グロス新規獲得必要社数 ＝ 純増 ＋ 解約補填", bold=True, fc=C_TOT)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 66, c, formula=f"=({col}65-{col}63)+{col}64", bold=True, fc=C_TOT, nf=NF_MAN, align="center")

sc(ws_a, 67, 1, "  月次新規獲得必要ペース（社/月）", italic=True)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 67, c, formula=f"=ROUND({col}66/12,1)", nf=NF_DEC, italic=True, align="center")

border_range(ws_a, 60, 67, 1, 4)
sc(ws_a, 68, 1,
   "⚠️ 注意：グロス新規獲得 ≠ 期末純増。Year3（月次解約2%）では解約補填に約11社/年が必要 → 月10社のグロス獲得目標。"
   "解約補填を見落とすと営業目標が過少設定になり、黒字化が遅延する。",
   italic=True, fc="FFF2CC")
ws_a.merge_cells("A68:D68")

# ─ セクション⑥: AI内部コスト・運転資本（WC）必要額試算
sec_hdr(ws_a, 70, "⑥ AI内部コスト（自社利用）＆ 運転資本（WC）必要額・黒字倒産リスク試算")
col_hdr(ws_a, 71)

sc(ws_a, 72, 1, "AI内部ツール月額（万円/月） ※注1")
for c, v in [(2, 5),(3, 10),(4, 20)]:
    sc(ws_a, 72, c, value=v, nf=NF_MAN, align="center")

sc(ws_a, 73, 1, "年間AI内部ツール費（万円）", bold=True, fc=C_SUB)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 73, c, formula=f"={col}72*12", bold=True, fc=C_SUB, nf=NF_MAN, align="center")

sc(ws_a, 74, 1,
   "※注1：Claude Code等のAPI費用はクライアントが直接負担。UNLIDが負担するのは"
   "社内開発・品質管理・FDE業務効率化ツールとしての自社利用分のみ（Claude Pro/Teams等）。",
   italic=True, fc=C_NOTE)
ws_a.merge_cells("A74:D74")

sc(ws_a, 75, 1, "売掛金 年末残高試算（万円） ＝ 売上×回収日数÷365", bold=True, fc=C_LOSS)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 75, c, formula=f"=ROUND({col}15*{col}42/365,0)",
       bold=True, fc=C_LOSS, nf=NF_MAN, align="center")

sc(ws_a, 76, 1, "資金立替期間（日）＝ 回収サイト − 支払サイト")
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 76, c, formula=f"={col}42-{col}43", nf=NF_MAN, align="center")

sc(ws_a, 77, 1, "⚠️ 運転資本必要額概算（万円）≒ 売上×（回収日−支払日）÷365", bold=True, fc=C_LOSS)
for c, col in [(2,"B"),(3,"C"),(4,"D")]:
    sc(ws_a, 77, c,
       formula=f"=ROUND({col}15*({col}42-{col}43)/365,0)",
       bold=True, fc=C_LOSS, nf=NF_MAN, align="center")

sc(ws_a, 78, 1, "ソフトウェア資産化オプション（参考）：システム開発費の一部をBS計上し減価償却",
   italic=True, fc=C_NOTE)
sc(ws_a, 78, 2, "PL費用処理（現行）", italic=True, fc=C_NOTE, align="center")
sc(ws_a, 78, 3, "BS無形資産計上（3年償却）検討可", italic=True, fc=C_NOTE, align="center")
sc(ws_a, 78, 4, "BS無形資産計上（3年償却）検討可", italic=True, fc=C_NOTE, align="center")

border_range(ws_a, 71, 78, 1, 4)
sc(ws_a, 79, 1,
   "⚠️ 黒字倒産リスク：Year3（売上5.7億）では売掛金が約4,700万円まで膨張しうる（回収30日設定）。"
   "入金サイト短縮（前払い化・月次請求）または調達額への運転資金上乗せが必要。"
   "プロジェクト型（160万/件×15件）は検収後払いのため、現金ショートに特に注意。"
   "BSの無形資産計上を選択すると営業黒字化タイミングが最大1〜2期前倒しになる。",
   italic=True, fc="FCE4D6")
ws_a.merge_cells("A79:D79")
...
# ─ 分析⑦: 解約率（Churn Rate）感度分析
sc(ws_sa, 79, 1,
   "【分析⑦】解約率（Churn Rate）感度分析——月次解約率の違いがグロス新規獲得目標に与える影響",
   bold=True, fc=C_BLUE, color="FFFFFF", align="center")
ws_sa.merge_cells("A79:H79")
sc(ws_sa, 80, 1,
   "ネット期末社数目標を固定した場合、月次解約率が上がるほどグロス新規獲得目標が増加する。"
   "Year2目標50社・Year3目標160社固定。",
   italic=True, fc=C_NOTE)
ws_sa.merge_cells("A80:H80")

for c,l in [(1,"月次解約率"),(2,"年次解約率"),(3,"Y2 解約補填社数"),(4,"Y2 グロス新規獲得"),(5,"Y2 月次ペース"),
            (6,"Y3 解約補填社数"),(7,"Y3 グロス新規獲得"),(8,"Y3 月次ペース")]:
    sc(ws_sa, 81, c, l, bold=True, fc=C_LBLUE, align="center")

churn_rates_sa = [0.01, 0.02, 0.03, 0.05, 0.07]
churn_base_row = None
for j, cr in enumerate(churn_rates_sa):
    row = 82 + j
    annual_cr = 1 - (1 - cr) ** 12
    y2_churn  = round(10 * annual_cr, 1)   # 期初10社×年次解約率
    y2_gross  = round((50 - 10) + y2_churn)
    y2_pace   = round(y2_gross / 12, 1)
    y3_churn  = round(50 * annual_cr, 1)   # 期初50社×年次解約率
    y3_gross  = round((160 - 50) + y3_churn)
    y3_pace   = round(y3_gross / 12, 1)
    is_base   = abs(cr - 0.025) < 0.001
    fc_row    = C_TOT if is_base else None
    label     = f"{cr*100:.0f}%{'（★ベース）' if is_base else ''}"
    sc(ws_sa, row, 1, label, bold=is_base, fc=fc_row, align="center")
    sc(ws_sa, row, 2, value=round(annual_cr, 4), nf=NF_PCT, bold=is_base, fc=fc_row, align="center")
    sc(ws_sa, row, 3, value=y2_churn, fc=fc_row, nf=NF_DEC, align="center")
    sc(ws_sa, row, 4, value=y2_gross, bold=is_base, fc=fc_row, nf=NF_MAN, align="center")
    sc(ws_sa, row, 5, value=y2_pace,  fc=fc_row, nf=NF_DEC, align="center")
    sc(ws_sa, row, 6, value=y3_churn, fc=fc_row, nf=NF_DEC, align="center")
    sc(ws_sa, row, 7, value=y3_gross, bold=is_base, fc=fc_row, nf=NF_MAN, align="center")
    sc(ws_sa, row, 8, value=y3_pace,  fc=fc_row, nf=NF_DEC, align="center")

border_range(ws_sa, 81, 81 + len(churn_rates_sa), 1, 8)
sc(ws_sa, 88, 1,
   "※ 月次解約率が3%を超えると、Year3グロス新規獲得目標が125社超/年（月10社+）に達する。"
   "「期末純増50社」の目標だけを追うと、解約補填が漏れて目標が実態より過少になる。",
   italic=True, fc=C_NOTE)
ws_sa.merge_cells("A88:H88")

# ─ 分析⑧: 期ズレ（タイムライン遅延）シナリオ
sc(ws_sa, 91, 1,
   "【分析⑧】期ズレ（タイムライン遅延）シナリオ——Year2売上達成が遅れた場合の現金ショートリスク",
   bold=True, fc=C_BLUE, color="FFFFFF", align="center")
ws_sa.merge_cells("A91:H91")
sc(ws_sa, 92, 1,
   "Year1年間固定費（人件費・インフラ・マーケ等）≒ 1,500万円。期初現金 = 2,500万円（増資2,000+借入500）想定。"
   "Year2目標達成が遅れると、その間の固定費が追加現金を消費する。",
   italic=True, fc=C_NOTE)
ws_sa.merge_cells("A92:H92")

for c,l in [(1,"シナリオ"),(2,"Year2達成時期"),(3,"Year1固定費（万円）"),(4,"追加固定費負担（万円）"),
            (5,"追加調達必要額（万円）"),(6,"期初現金残（万円）"),(7,"現金ショート月"),(8,"対応策")]:
    sc(ws_sa, 93, c, l, bold=True, fc=C_LBLUE, align="center")

delay_scenarios = [
    (94, "ベース（定常）",       "Year2末（Month24）",  1500, 0,    0,    1000, "なし",         C_PROFT),
    (95, "遅延6ヶ月",            "Year2.5（Month30）", 1500, 750,  750,  250,  "Month28〜30",  C_TOT),
    (96, "遅延12ヶ月（Year3着地）","Year3（Month36）", 1500, 1500, 1500, -500, "Month26〜28",  C_LOSS),
]
for row, label, timing, fixed, extra, add_raise, cash_end, shock_month, fc in delay_scenarios:
    sc(ws_sa, row, 1, label, bold=True, fc=fc)
    sc(ws_sa, row, 2, timing, fc=fc, align="center")
    sc(ws_sa, row, 3, value=fixed, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 4, value=extra, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 5, value=add_raise, bold=True, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 6, value=cash_end, bold=True, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 7, shock_month, bold=True, fc=fc, align="center")
    cope = "追加調達不要" if add_raise == 0 else f"追加{add_raise}万円調達 or 費用削減"
    sc(ws_sa, row, 8, cope, fc=fc, align="center")

border_range(ws_sa, 93, 96, 1, 8)
sc(ws_sa, 97, 1,
   "⚠️ 単価未達よりも『タイムライン遅延』が死活問題。6ヶ月の遅延で750万円の追加資金が必要になる。"
   "調達時に「12〜18ヶ月のランウェイ」を確保しておくことで6ヶ月遅延でも生存できる。",
   italic=True, fc="FFF2CC")
ws_sa.merge_cells("A97:H97")

# ─ 分析⑨: CAC（顧客獲得コスト）悪化シナリオ
sc(ws_sa, 100, 1,
   "【分析⑨】CAC（顧客獲得コスト）悪化シナリオ——マーケ費・採用費高騰時のLTV/CAC・営業利益影響",
   bold=True, fc=C_BLUE, color="FFFFFF", align="center")
ws_sa.merge_cells("A100:H100")
sc(ws_sa, 101, 1,
   "Year2 ベースCAC ＝（採用費500万＋マーケ費360万）÷50社 ＝ 17.2万/社。"
   "LTV ＝ 月20万×12ヶ月＋人材紹介140万 ＝ 380万/社。アーリー層枯渇後のCAC悪化に備える。",
   italic=True, fc=C_NOTE)
ws_sa.merge_cells("A101:H101")

for c,l in [(1,"シナリオ"),(2,"Year2 CAC（万円）"),(3,"Year2 LTV/CAC"),(4,"Year2 追加費用（万円）"),
            (5,"Year2 営業利益影響（万円）"),(6,"Year3 CAC（万円）"),(7,"Year3 LTV/CAC"),(8,"判定")]:
    sc(ws_sa, 102, c, l, bold=True, fc=C_LBLUE, align="center")

# Base CAC計算: Year2=(500+360)/50=17.2 Year3=(1500+600)/160=13.1
cac_scenarios = [
    (103, "ベース（1.0x）",  17.2, round(380/17.2, 1),  0,    0,    13.1, round(380/13.1, 1), C_PROFT),
    (104, "CAC 1.5x",       25.8, round(380/25.8, 1),  430,  -430, 19.7, round(380/19.7, 1), C_TOT),
    (105, "CAC 2.0x（限界）", 34.4, round(380/34.4, 1), 860, -860, 26.3, round(380/26.3, 1), C_LOSS),
]
for row, label, y2cac, y2ratio, y2extra, y2impact, y3cac, y3ratio, fc in cac_scenarios:
    sc(ws_sa, row, 1, label, bold=True, fc=fc)
    sc(ws_sa, row, 2, value=y2cac,   bold=True, fc=fc, nf=NF_DEC, align="center")
    sc(ws_sa, row, 3, value=y2ratio, bold=True, fc=fc, nf=NF_DEC, align="center")
    sc(ws_sa, row, 4, value=y2extra, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 5, value=y2impact, bold=True, fc=fc, nf=NF_MAN, align="center")
    sc(ws_sa, row, 6, value=y3cac,   fc=fc, nf=NF_DEC, align="center")
    sc(ws_sa, row, 7, value=y3ratio, fc=fc, nf=NF_DEC, align="center")
    ok = "優良（Unit Econ成立）" if y2ratio >= 10 else "要見直し（LTV/CAC<10x）"
    sc(ws_sa, row, 8, ok, bold=True, fc=fc, align="center")

border_range(ws_sa, 102, 105, 1, 8)
sc(ws_sa, 106, 1,
   "※ CAC 2x まではLTV/CAC ≥ 11x を維持できる（投資家の一般基準 ≥ 3x を大幅超過）。"
   "人材紹介フィー140万/件が『CAC回収の保険』として機能するため、Unit Econ悪化耐性が高い。"
   "ただし月次マーケ費をYear3で月50万超に増やす際は、CAC測定と月次レビューを徹底すること。",
   italic=True, fc=C_NOTE)
ws_sa.merge_cells("A106:H106")
...
# 保存
# ══════════════════════════════════════════════
output_path = "/home/user/UNLID/UNLID_財務三表_v4.1.xlsx"
wb.save(output_path)
print(f"✅ 作成完了: {output_path}")
print("シート一覧:")
for s in wb.sheetnames:
    print(f"  ・{s}")
print("\n v4.1 変更内容（財務プロフェッショナル視点での解像度向上）:")
print("  ・前提条件 ⑤ 解約率（Churn Rate）＆ グロス新規獲得計画 を追加")
print("    ↳ 月次解約率 / 年次解約率 / 期中解約予測社数 / グロス新規獲得必要社数 / 月次ペース")
print("  ・前提条件 ⑥ AI内部コスト・運転資本（WC）必要額試算 を追加")
print("    ↳ AI内部ツール費（自社利用分のみ・クライアントAPI費は顧客負担）")
print("    ↳ 売掛金膨張・資金立替期間・運転資本必要額・ソフトウェア資産化オプション注記")
print("  ・感度分析 ⑦ 解約率感度分析（グロス獲得目標への影響）を追加")
print("  ・感度分析 ⑧ 期ズレ（タイムライン遅延）シナリオ を追加")
print("    ↳ 6ヶ月・12ヶ月遅延時の現金ショートリスクと追加調達必要額")
print("  ・感度分析 ⑨ CAC悪化シナリオ（1.5x / 2x）を追加")
print("    ↳ LTV/CAC比率・営業利益影響・Unit Econ成立判定")
