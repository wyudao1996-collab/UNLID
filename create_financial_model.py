# -*- coding: utf-8 -*-
"""UNLID 財務三表（PL/BS/CF）+ 感度分析 v2.0
新規追加：法定福利費（社会保険料）/ 政策公庫借入500万・利息・返済 / EBT計算
"""
...
title_row(ws_a, 1, "UNLID 財務モデル 前提条件",
          "作成日：2026年3月  |  単位：万円  |  中央値シナリオ")
...
A1 = [
    ...
    (7,  "FDE受託 月額単価（万円/社）",         15,    18,    20,  False),
...
sc(ws_sa, 9+len(growth_rates)+1, 1,
   "※ 現状ベース：成長倍率≒5x（Year1:2,220万→Year2:14,400万想定）、原価率≒8%（ワーカー報酬率）",
   italic=True, fc=C_NOTE)
...
for j, co in enumerate(companies):
    ...
    for i, p in enumerate(prices):
        ...
        sc(ws_sa, row, col, formula=formula, nf=NF_MAN, align="center",
           fc="FFFACC" if co==50 and p==18 else None)
...
scenarios = [
    (42, "楽観（Upside）",
     round(2220*1.3), round(-120+200), round(14400*1.4), round(4500*1.7), round(47800*1.3), round(18500*1.5),
     C_PROFT),
    (43, "ベース（中央値）",
     ...
     C_TOT),
    (44, "保守（Downside）",
     round(2220*0.7), round(-120*2), round(14400*0.5), round(4500*0.2), round(47800*0.55), round(18500*0.35),
     C_LOSS),
]
for row, label, v1,v2,v3,v4,v5,v6, fc in scenarios:
    ...
    if row == 43:
        ...
        sc(ws_sa, row, 8, value=round((18500*1.5*0.7)*3/4), bold=True, fc=fc, nf=NF_MAN, align="center")
    else:
        sc(ws_sa, row, 8, value=round((18500*0.35*0.7)*3/4), bold=True, fc=fc, nf=NF_MAN, align="center")
