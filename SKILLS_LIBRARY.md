...
### Notion — ワークスペース・ナレッジ管理

**インストール：** `/plugin install notion@anthropic-agent-skills`

**できること：**
- ページの作成・編集・削除
- データベースへのレコード追加・フィルタリング・ソート
- ブロック単位のコンテンツ操作（テキスト・テーブル・トグル等）
- ワークスペース内の検索・ページ一覧取得
- コメントの追加・取得

**使い方の例：**
```
今日のミーティング議事録をNotionの「議事録」データベースに追加して
「プロジェクト管理」ページに新しいタスクを追加して
先週のページ一覧を取得して
```
...
**インストール：** `/plugin install asana@anthropic-agent-skills`

**できること：**
- タスクの作成・更新・完了・削除
- プロジェクトへのタスク追加・担当者割り当て
- 期限・優先度の設定
- プロジェクトの進捗状況取得
- タスクへのコメント追加
- サブタスクの作成・管理
...
**使い方の例：**
```
「LP制作」プロジェクトに「デザイン確認」タスクを期限3/25で追加して
田中さんに未完了のタスクを一覧で見せて
今週期限のタスクをすべて取得して
```
...
**インストール：** `/plugin install atlassian@anthropic-agent-skills`

**できること（Jira）：**
- イシュー（課題）の作成・更新・ステータス変更
- スプリントへのイシュー追加
- 担当者割り当て・優先度設定
- イシューへのコメント追加
- フィルタによる課題一覧取得
...
**使い方の例：**
```
JiraにBUG-001「ログインエラー」のイシューを作成して重要度をHighに設定して
Confluenceの「技術仕様」スペースに新しいページを作って
現在のスプリントの未完了イシューを一覧で見せて
```

---

### Canva — デザイン作成・テンプレート操作

**インストール：** `/plugin install canva@anthropic-agent-skills`

**できること：**
- テンプレートの検索・適用
- デザインの作成・複製
- テキスト・画像要素の編集
- デザインのエクスポート（PNG・PDF・MP4）
- ブランドキットの参照・適用
- プレゼンテーション・SNS投稿・バナー等の生成

**使い方の例：**
```
「会社紹介」テンプレートを使ってA4縦のデザインを作って
このデザインをPDFでエクスポートして
SNS用の正方形バナーを作成して
```
...
**インストール：** `/plugin install figma@anthropic-agent-skills`

**できること：**
- Figmaファイルのコンポーネント一覧取得
- レイヤー構造・スタイル情報の読み取り
- デザイントークン（色・タイポグラフィ・スペーシング）の抽出
- フレーム・コンポーネントの画像エクスポート
- コメントの取得・追加
- デザインからCSSコードの生成

**使い方の例：**
```
このFigmaファイルのカラートークンをすべて抽出して
「ボタン」コンポーネントのCSSを生成して
デザインのフィードバックコメントを一覧で見せて
```

---

### Sentry — エラー監視・インシデント管理

**インストール：** `/plugin install sentry@anthropic-agent-skills`

**できること：**
- エラー・例外の一覧取得・詳細確認
- イシューのステータス変更（解決済み・無視等）
- リリース情報の取得
- パフォーマンス指標の確認
- アラートルールの確認
- エラー発生頻度・トレンドの分析
...
**使い方の例：**
```
直近24時間で最も多く発生しているエラーを教えて
「TypeError: Cannot read properties」のイシューを解決済みにして
最新リリースのエラー件数を前バージョンと比較して
```

---

### Zapier — ワークフロー自動化（5,000+アプリ連携）

**インストール：** `/plugin install zapier@anthropic-agent-skills`

**できること：**
- Zapのトリガー設計・アクション設定
- 既存Zapの一覧取得・有効化/無効化
- 5,000以上のアプリ間のデータ連携フロー構築
- Webhook・フィルター・フォーマッターの設定
- Zapの実行履歴・エラーログの確認
- マルチステップZapの設計

**使い方の例：**
```
Googleフォームに回答があったらSlackに通知するZapを作って
現在有効なZapの一覧を見せて
失敗したZapのエラーログを確認して
```
